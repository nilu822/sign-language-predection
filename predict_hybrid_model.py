...
model = load_model('model_hybrid/sign_model_hybrid.keras')
with open('model_hybrid/labels.txt', 'r') as f:
...
def predict_hybrid(frame):
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            img_input = cv2.resize(hand_crop, (IMG_SIZE, IMG_SIZE))
            ...
            if prediction_count == 0:
                prediction = model.predict([img_input, landmark_input], verbose=0)[0]
                    ...
                    if consistency_counter >= required_consistency:
                        if current_time - recent_prediction_time > repeat_delay:
                            ...
                            if typed_text.strip() != old_text:
                                spoken_once = False
                                last_spoken_text = ""  # ✅ reset for next speech
...
# CAM_WIDTH = 960
# CAM_HEIGHT = 480
# IMG_SIZE = 64 # Expected input size for hybrid model images

# def extract_landmarks(landmarks, frame_shape):
#     h, w = frame_shape
#     return np.array([[lm.x, lm.y] for lm in landmarks.landmark]).flatten()

# # UPDATED: Directly modify typed_text and history, and reset spoken_once
# def reset_text():
#     global typed_text, history, prev_prediction, consistency_counter, flash_timer, recent_prediction_time, spoken_once, speak_thread_running
#     print("[HYBRID DEBUG] reset_text() called - performing immediate reset")
#     typed_text = ""
#     history = []
#     prev_prediction = ""
#     consistency_counter = 0
...
# # UPDATED: Directly modify typed_text and history
# def backspace_text():
#     global typed_text, history, spoken_once
#     print("[HYBRID DEBUG] backspace_text() called - performing immediate backspace")
#     if history:
#         history.pop()
#         typed_text = " ".join(history) + " "
...
#             # Extract hand ROI for image input
#             hand_crop = frame[y_min:y_max, x_min:x_max]
#             if hand_crop.size == 0:
#                 continue

#             # Resize to expected input size for the CNN part of the hybrid model
#             img_input = cv2.resize(hand_crop, (IMG_SIZE, IMG_SIZE))
#             img_input = np.expand_dims(img_input, axis=0) / 255.0  # Normalize to 0-1

#             # Extract landmarks for landmark input
#             landmark_input = extract_landmarks(hand_landmarks, (h, w))
#             if landmark_input.shape != (42,):
#                 continue
#             landmark_input = np.expand_dims(landmark_input, axis=0)
...
# def preprocess_hand_crop(image):
#     gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#     resized = cv2.resize(gray, (64, 64))
#     return np.expand_dims(resized, axis=(0, -1)) / 255.0
...
#             if landmark_input.shape != (42,):
#                 continue
#             image_input = preprocess_hand_crop(roi_display)

#             # Predict
#             if prediction_count == 0:
#                 landmark_input_exp = np.expand_dims(landmark_input, axis=0)
#                 prediction = model.predict([landmark_input_exp, image_input], verbose=0)[0]
#                 confidence = np.max(prediction)
#                 label = labels[np.argmax(prediction)]
#                 prediction_buffer.append(label)
