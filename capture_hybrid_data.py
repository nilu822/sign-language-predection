# capture_hybrid_data.py
# # capture_hybrid_data.py

### ✅ Updated `capture_hybrid_data.py`
import os
import cv2
import time
import numpy as np
...
def capture_gesture_data(word, mode="hybrid"):
    ...
    while sample_count < total_samples:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                cv2.imwrite(os.path.join(PREVIEW_PATH, f"{file_index}.jpg"), roi_resized)
...
# def capture_gesture_data(word, mode="hybrid"):
#     print(f"[INFO] Capturing gesture for: {word} ({mode})")
#     speak(f"Capturing gesture for {word} in hybrid mode")
...
#         cv2.imshow("Hybrid Data Capture", frame)
...
#     speak(f"{word} gesture captured successfully in hybrid mode")
...
#this was the original code - backend code --> working  
# import os
...
# # === Folder setup ===
# BASE_PATH = 'dataset_hybrid/word_hybrid'
# os.makedirs(BASE_PATH, exist_ok=True)

# # === Class name input and reset logic ===
# while True:
#     print("\n[INFO]:- Enter the class name to capture (e.g., Thankyou): ", end="", flush=True)
...
#     cv2.imshow("Hybrid Hand Capture", frame)
#     if cv2.waitKey(1) & 0xFF == ord('q'):
#         break
